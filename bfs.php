...
<?php
	...
	class Graf {
		...
		public function BFS($mulai, $tujuan){
			$queue = array(array($mulai));
			$visited = array();

			while (!empty($queue)) {

				// masukkan antrian paling depan ke variabel jalur
				$jalur = array_shift($queue);
        		...
        		// jika state tidak sama dengan tujuan, maka cek apakah state tidak ada di visited
        			...
        			foreach ($this->peta1[$state] as $cabang) { //cek semua cabang dari state
		                ...
		                array_push($queue,$jalur_baru);  #update/tambah queue dengan jalur_baru
        			}
      ...
      <div class="form-login">
        ...
        <div class="form">
            <?php
            	$g = new Graf($peta1);
				$sep = '';
				foreach ($g->BFS($Start,$Finis) as $key => $value) {
					echo $sep, $value;
					$sep = ' -> ';
				}
            ?>
